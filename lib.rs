...
#[cfg(feature="unstable")] pub fn line_no_blend(mut target: Image<&mut[rgba8]>, p0: vec2, p1: vec2, color: rgba8) {
	assert!(p0 != p1);
	let size = target.size;
	let mut f = |p,_| if p < target.size { target[p] = color; };
...
#[cfg(feature="unstable")] pub fn line(eotf: &[f32; 256], oetf: &[u8; 0x1000], target: &mut Image<&mut[rgba8]>, p0: vec2, p1: vec2, color: rgbf) {
	assert!(p0 != p1);
	let size = target.size;
...
#[cfg(feature="unstable")] pub fn parallelogram(target: &mut Image<&mut[f32]>, top_left: vec2, bottom_right: vec2, descending: bool, vertical_thickness: f32, opacity: f32) {
